...
\caption{Nicht-normalisiertes spektrales Clustering~\cite{Luxburg07}}
	...
	Bestimme die Eigenvektoren $u_1, \dots, u_k$ zu den $k$ kleinsten Eigenwerten von $L$\;
	...
	Clustere die Punkte $(y_i)_{i \in \{1, \dots, n\}}$ mit einem $k$-means-Algorithmus in Cluster $C_1, \dots, C_k$
\end{algorithm}
Die beiden folgenden Algorithmen verwenden die normalisierten Laplace-Matrizen und wurden in den jeweils zitierten Papieren
vorgestellt.
...
\caption{Normalisiertes spektrales Clustering~\cite{ShiM00}}
	...
	Bestimme die Eigenvektoren $u_1, \dots, u_k$ zu den $k$ kleinsten Eigenwerten von $\mathcal{L}_{rw}$\;
	Sei $U \in \mathbb{R}^{n \times k}$ die Matrix mit $u_1, \dots u_k$ als Spaltenvektoren\;
	Für $i \leftarrow \{ 1, \dots, n \}$ sei $y_i \in \mathbb{R}^k$ der Vektor der $i$-ten Reihe von $U$\;
	Clustere die Punkte $(y_i)_{i \in \{1, \dots, n\}}$ mit einem $k$-means-Algorithmus in Cluster $C_1, \dots, C_k$
\end{algorithm}
...
\caption{Normalisiertes spektrales Clustering~\cite{NgJW01}}
	...
	Bestimme die Eigenvektoren $u_1, \dots, u_k$ zu den $k$ kleinsten Eigenwerten von $\mathcal{L}_{sym}$\;
	Sei $U \in \mathbb{R}^{n \times k}$ die Matrix mit $u_1, \dots u_k$ als Spaltenvektoren\;
